...
# Some functions will enable processing a prompt in chunks to bypass the tokens limit.
import openai
import nltk
nltk.download('punkt')
...
class OpenaiLongParser():
    ...
    def custom_word_tokenize(self, text):
        """Tokenizes a string. Currently using a simpler version of the nltk word_tokenize function.
        Args:
    ...
    def break_up_tokens_in_chunks(self, tokens, chunk_size):
        ...
        if len(tokens) <= chunk_size:
            ...
            yield chunk
            yield from self.break_up_tokens_in_chunks(tokens[end_idx:], chunk_size)
    ...
    def break_up_longtext_to_chunks(self, text, chunk_size):
        ...
        self.chunks = [self.convert_to_detokenized_text(local_tokens) \
                       for local_tokens in self.break_up_tokens_in_chunks(tokens, chunk_size)]

    def call_chatGPT(self, prompt, temperature=0.1, presence_penalty=0.0, frequency_penalty=0.0):
        ...
        NbTokensInResponse = self.count_tokens([response.choices[0].message.content])
    ...
    def process_chunks_through_prompt(self, prompt, save_path=None, temperature=0.1, presence_penalty=0.0, frequency_penalty=0.0):
        ...
        for i, chunk in enumerate(list_chunk):
            ...
            result = self.call_chatGPT(submit_prompt, temperature=temperature, presence_penalty=presence_penalty, frequency_penalty=frequency_penalty)
            if save_path:
