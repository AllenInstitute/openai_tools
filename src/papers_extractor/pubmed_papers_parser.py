...
class PubmedPapersParser:
    ...
    def search_pubmed(self, max_results=1000):
        ...
        if self.ids:
            ...
            # We want to tell the user if there was more than one page of results
    ...
    def fetch_details(self):
        ...
        if self.details:
            logging.warning("Details already fetched, returning cached details")
            return self.details
    ...
    def get_list_unique_papers(self, local_database=None):
        ...
        for detail in self.details:
            if detail['doi'] is None:
                ...
                identity = detail['doi']
            try:
                unique_paper = UniquePaper(identity, local_database=local_database)
                ...
                logging.warning("Could not create unique paper for {}".format(identity))
    ...
    def _parse_article(self, article):
        ...
        doi_elem = article.find('PubmedData/ArticleIdList/ArticleId[@IdType="doi"]')
        pmid = article.find('MedlineCitation/PMID').text
        authors = [
            f"{elem.find('ForeName').text if elem.find('ForeName') is not None else ''} "
            f"{elem.find('LastName').text if elem.find('LastName') is not None else ''}".strip() 
